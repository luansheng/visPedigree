...
- ped:
  ...
  results, use `pedmatrix(ped_split$GP1, ...)` to process individual
  groups.
...
Returns a matrix or vector with S3 class `"pedmatrix"`.
...
- [`summary_pedmatrix`](https://luansheng.github.io/visPedigree/reference/summary_pedmatrix.md)`(x)`:
...
`pedmatrix()` separately for each method.
...
- [`expand_pedmatrix`](https://luansheng.github.io/visPedigree/reference/expand_pedmatrix.md)`(x)`:
...
#>  [1] "i"            "p"            "Dim"          "Dimnames"     "x"           
#>  [6] "uplo"         "factors"      "class"        "call_info"    "ped"         
#> [11] "pedmatrix_S4"
