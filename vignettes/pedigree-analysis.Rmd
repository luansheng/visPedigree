...
These are the exponentials of Shannon entropy, which give a diversity measure
that counts all contributors — including rare ones — more equitably than the
classical $q = 2$ metric. They appear in the `pediv()` summary as `feH` and
`faH`, and in the `pedcontrib()` summary as `f_e_H` and `f_a_H`.
...
considered. When $f_e^{(H)} / f_e$ is much larger than 1, many minor founders
still carry genetic material into the reference population but are invisible to
...
3. Quantify diversity loss with `fe`, `fa`, and `fg`; compare with `feH` and
   `faH` to assess long-tail founder value.
