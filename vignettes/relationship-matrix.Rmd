...
The matrices returned by `pedmat()` are ordinary matrix-like R objects, so they can be used directly in downstream matrix-vector or matrix-matrix operations. For example, a vector of candidate contributions or breeding values can be multiplied by the additive relationship matrix, and inverse matrices can be used in mixed-model or genetic-evaluation workflows.

```{r linear_algebra}
# A simple matrix-vector multiplication
weights <- rep(1 / nrow(mat_A), nrow(mat_A))
names(weights) <- rownames(mat_A)
head(as.vector(mat_A %*% weights))

# A small matrix-matrix cross-product example
X <- model.matrix(~ factor(tped$Gen))
head(crossprod(X, as.matrix(mat_A) %*% X))
```

For very large pedigrees, avoid expanding dense matrices unless it is necessary for the analysis. Use `Ainv`, compact matrices, or grouped summaries when the full relationship matrix would exceed available memory.
