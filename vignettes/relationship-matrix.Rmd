...
1.4 [Using Matrices in Downstream Linear Algebra](#1-4)
...
### 1.4 Matrix-Free Products for Downstream Analysis {#1-4}
...
$A^{-1}x$, or $A^{-1}X$ directly from the pedigree.

The implementation uses the pedigree factorization $A = TDT'$ and the
indirect traversal described by Colleau (2002). It requires linear storage for
a vector right-hand side and never materializes the $n \times n$ matrix $A$.
...
# AX: compare two contribution schemes in one traversal
schemes <- cbind(
  Equal = c(Z1 = 0.5, Z2 = 0.5),
  Z1_only = c(Z1 = 1, Z2 = 0)
)
head(pedprod(tped, schemes))
```

The explicit and matrix-free calculations are numerically equivalent:

```{r linear_algebra_equivalence}
weights_full <- setNames(numeric(nrow(mat_A)), rownames(mat_A))
weights_full[names(weights)] <- weights
all.equal(
  unname(Ax),
  unname(as.vector(mat_A %*% weights_full)),
  tolerance = 1e-12
)
```

For very large pedigrees, use `pedprod()` whenever the analysis only requires a
matrix product. Construct `Ainv`, compact matrices, or grouped summaries only
when the corresponding matrix entries are themselves required.
