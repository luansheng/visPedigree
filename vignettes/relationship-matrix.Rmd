...
```{r pedprod_AX}
# Compare three contribution schemes side by side
schemes <- cbind(
...
# Relationship differences among individuals across schemes
AX[tped$Ind[1:6], ]
```

Each column is independent — use this when evaluating many candidate sets in
optimal contribution selection, or when the right-hand side is a design matrix
in a mixed model. For an $n \times p$ matrix, the traversal cost is $O(np)$,
compared to $O(p \cdot n^2)$ for explicit products with a precomputed $A$.
...
Trace the expected genetic contribution of a founder group to all descendants.
This reveals which founders have the largest genetic footprint in the current
population:

```{r pedprod_geneflow}
# Founders contribute equally to a hypothetical gene pool
founder_ids <- tped[Gen == 1, Ind]
founder_contrib <- setNames(
  rep(1 / length(founder_ids), length(founder_ids)),
  founder_ids
)
founder_rel <- pedprod(tped, founder_contrib)

# Descendants most related to the founder pool (top 5)
tail(sort(founder_rel), 5)

# Descendants least related (bottom 5)
head(sort(founder_rel), 5)
```
