...
1.  **[`pedexport()`](https://luansheng.github.io/visPedigree/reference/pedexport.md)
    for breeding software formats**: New function
    [`pedexport()`](https://luansheng.github.io/visPedigree/reference/pedexport.md)
    converts a `tidyped` pedigree into the file format of common animal
    and plant breeding programs - BLUPF90, ASReml, Echidna, WOMBAT,
    MTDFREML, DMU - plus a generic `numeric` layout and an in-memory
    `sommer` format. Character formats (`asreml`, `echidna`, `wombat`,
    `sommer`) keep the original character IDs; numeric formats
    (`blupf90`, `mtdfreml`, `dmu`, `numeric`) renumber the pedigree and
    carry an `xref` mapping back to the original IDs (attribute on the
    returned table, and a `<file>.xref` file when `file` is given),
...
1.  **Safer
    [`pedexport()`](https://luansheng.github.io/visPedigree/reference/pedexport.md)
    files**: Export validation now rejects unsupported or malformed
    separators, invalid file paths, and unquoted identifiers or
    missing-parent symbols that would split into extra fields. Partially
    missing numeric index columns are rebuilt as a consistent
    `IndNum`/`SireNum`/`DamNum` set, while `sommer` remains an
    in-memory-only format.
2.  **Sommer relationship-matrix compatibility**: Documented and tested
    the complete workflow using
    `pedmat(..., method = "A", sparse = FALSE)`. This returns the base R
    matrix required by `sommer::vsr(Gu = A)`, with row and column names
    aligned to `pedexport(..., software = "sommer")$ID`.
...
1.  **[`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    ...
    now writes SVG files when `file` ends in `.svg`; other file names
    continue to use PDF output by default.
2.  **[`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    custom labels**: Added a `labelvar` argument to display labels from
    either a user-selected pedigree column or a row-aligned character
    vector. Compact full-sib family nodes continue to show family size.
3.  **[`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    symbol schemes**: Added a `shapeby` argument for choosing how node
    shapes are encoded. The default `shapeby = "sex"` uses circles for
    females, squares for males, diamonds for unknown sex, and hexagons
    for monoecious individuals. Set `shapeby = "role"` to keep the
    legacy role-based symbol scheme in which real individuals are
    circles and compact full-sib family summaries are rectangles.
4.  **Matrix-free relationship products**: Added
...
1.  **Matrix-free
    ...
    summaries**:
    [`pedrel()`](https://luansheng.github.io/visPedigree/reference/pedrel.md)
    now computes grouped mean relationships and coancestries from
...
2.  **Large-group support in
    ...
    no-op arguments were removed before release, while `compact` remains
...
3.  **Matrix-free coancestry analyses**: `pedne(method = "coancestry")`
    ...
    reuses the same products for $`N_e`$ and founder genome equivalents,
    while
...
1.  **[`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    generation labels**: Added a `genlab` argument to
    [`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
...
1.  **Inbreeding calculation**: Replaced the Meuwissen and Luo (1992)
    linear path-trace algorithm in `cpp_calculate_inbreeding()` with the
    Sargolzaei and Iwaisaki (2005) LAP (Longest Ancestral Path) bucket
    method. At N = 1,000,000, the C++ kernel completes in about 0.15 s
    (previously about 15 s), and the full
    [`inbreed()`](https://luansheng.github.io/visPedigree/reference/inbreed.md)
    call returns in about 0.40 s. The implementation uses O(1) ancestor
    retrieval via bucket pop, O(1) duplicate suppression via the
    `L[k] == 0` check, and O(m_i) path-coefficient reset. Results are
    numerically identical to the previous implementation (maximum
    difference \< 2e-15).
2.  **[`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md)
    candidate tracing**: When the input is already a `tidyped` object
    and `cand` is specified, the fast path now uses three C++ BFS
    functions (`cpp_trace_ancestors()`, `cpp_trace_descendants()`, and
    `cpp_topo_order()`) instead of rebuilding an `igraph` object. At N =
    1,000,000 with 200 candidates, elapsed time drops from about 0.91 s
    to about 0.056 s. The fast path now also applies sibling and mate
...
3.  **[`pedgenint()`](https://luansheng.github.io/visPedigree/reference/pedgenint.md)
    ...
    representative dataset: 4.24 s to 0.49 s.
4.  **`pedmat(sparse = TRUE)` matrix coercion**: The conversion from a
    dense numeric matrix to a `Matrix::dgeMatrix` now bypasses the S4
    dispatch overhead of `as(mat, "dgeMatrix")` via a direct
    [`methods::new()`](https://rdrr.io/r/methods/new.html) call backed
    by a package-level class cache. Benchmark on a representative
...
2.  **[`pediv()`](https://luansheng.github.io/visPedigree/reference/pediv.md)
    ...
    `merge(..., by = "Ind", all.x = TRUE)` calls sorted output
    alphabetically by `Ind`, breaking the `IndNum == row-index`
    invariant used by the fast-path BFS in
...
1.  **[`pediv()`](https://luansheng.github.io/visPedigree/reference/pediv.md)
    ...
    `GeneDiv = 1 - MeanCoan` column, the pedigree-based retained genetic
    diversity of the reference population. Values lie in $`[0, 1]`$,
    with larger values indicating more diversity retained relative to an
    unrelated base population.
    [`print.pediv()`](https://luansheng.github.io/visPedigree/reference/print.pediv.md)
    displays it alongside `fg` and `MeanCoan`.
2.  **[`vismat()`](https://luansheng.github.io/visPedigree/reference/vismat.md)
    ...
    no longer attempts a full N × N matrix expansion. It uses the
...
1.  **[`vismat()`](https://luansheng.github.io/visPedigree/reference/vismat.md)
    ...
    `VISMAT_WARN_THRESHOLD`) at the top of `R/vismat.R`.
...
1.  **Diversity notation**: Output columns in `pedhalflife()$timeseries`
    are now lowercase (`fe`, `fa`, `fg`, `lnfe`, `lnfa`, `lnfg`,
    `lnfafe`, and `lnfgfa`) to match population genetics notation.
2.  **[`plot.pedhalflife()`](https://luansheng.github.io/visPedigree/reference/pedhalflife.md)
    log view**: In `type = "log"` mode, the plot includes an OLS
    regression line for total diversity decay
    ($`\ln f_g \sim \text{Time}`$) and a vertical reference line for the
    diversity half-life $`T_{1/2}`$.
3.  **Time-unit labeling**:
    ...
    now use the name of the `timevar` column, such as `Gen` or `Year`,
    for axis and summary labels.
...
1.  **Information-theoretic diversity half-life
    ...
    Added
    [`pedhalflife()`](https://luansheng.github.io/visPedigree/reference/pedhalflife.md)
    to track $`f_e`$, $`f_a`$, and $`f_g`$ across time points and fit a
    log-linear decay model for the rate of genetic diversity loss. The
    total loss rate $`\lambda_{total}`$ is decomposed into foundation
    bottleneck ($`\lambda_e`$), breeding bottleneck ($`\lambda_b`$), and
    genetic drift ($`\lambda_d`$). The diversity half-life
    $`T_{1/2} = \ln 2 / \lambda_{total}`$ is reported in units of the
    `timevar` column. S3 [`print()`](https://rdrr.io/r/base/print.html)
    and [`plot()`](https://rdrr.io/r/graphics/plot.default.html) methods
    are provided, with both log-scale (`type = "log"`) and raw-scale
    (`type = "raw"`) views.
...
1.  **Shannon-entropy effective founders and ancestors (`feH`, `faH`)**:
    ...
    number of order $`q = 1`$ (Shannon entropy): `feH`, the effective
    number of founders under equal entropy weighting, and `faH`, the
    effective number of ancestors under equal entropy weighting. They
    satisfy $`N_{\mathrm{Founder}} \ge f_e^H \ge f_e`$ and
    $`N_{\mathrm{Ancestor}} \ge f_a^H \ge f_a`$, respectively. Both are
    computed as $`\exp(-\sum p_i \ln p_i)`$ and complement the classical
    quadratic ($`q = 2`$) effective numbers $`f_e`$ and $`f_a`$ (Lacy
    1989; Boichard et al. 1997).
...
1.  **Fail-fast incomplete-pedigree analysis**:
    ...
    incorrect results, such as zero inbreeding, caused by calculating on
...
1.  **`tidyped` class redesign**: Refined the internal `tidyped` class
    architecture around a clearer metadata contract and safer
    S3/data.table interaction model for repeated downstream analysis and
    extension.
2.  **Safer `tidyped` workflows**: Added
    ...
    to make class checks and metadata inspection explicit.
...
1.  **S3 class protection**: Added
    ...
    and an internal
    [`ensure_tidyped()`](https://luansheng.github.io/visPedigree/reference/ensure_tidyped.md)
    mechanism to handle cases where R operations such as
...
1.  **Analysis entry points**: Updated core analysis functions,
    ...
    and
    [`pedrel()`](https://luansheng.github.io/visPedigree/reference/pedrel.md),
    to use the class-recovery logic.

## Changes in version 1.3.4 released on 14 Mar 2026

### Bug fixes

1.  **`data.table` return visibility**: Functions returning `data.table`
    or `tidyped` objects now explicitly return with `[]` so results
    auto-print in the R console and knitted documents after internal
    `data.table` operations such as `:=` and `set*`. Affected functions
    included
...
1.  **Analysis regression coverage**: Added unit tests verifying that
    ...
    `compact = FALSE` on the same pedigree.

## Changes in version 1.3.2 released on 13 Mar 2026

### New features

1.  **Core analysis examples**: Added `@examples` to
    [`pedne()`](https://luansheng.github.io/visPedigree/reference/pedne.md),
    [`pedecg()`](https://luansheng.github.io/visPedigree/reference/pedecg.md),
    and
    [`pedsubpop()`](https://luansheng.github.io/visPedigree/reference/pedsubpop.md).
2.  **Pedigree connectivity analysis
    ([`pedsubpop()`](https://luansheng.github.io/visPedigree/reference/pedsubpop.md))**:
    Enhanced
    [`pedsubpop()`](https://luansheng.github.io/visPedigree/reference/pedsubpop.md)
    to distinguish pedigree splitting via
...
1.  **[`vispstat()`](https://luansheng.github.io/visPedigree/reference/vispstat.md)
    ...
    now uses explicit `%in% c("SS", "SD", "DS", "DD")` subsetting.

## Changes in version 1.3.1 released on 12 Mar 2026

### New features

1.  **Selfing support in
    [`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md)**:
    Added the `selfing` argument to support plant and aquaculture
    pedigrees where an individual can appear as both sire and dam,
    resolving sex-conflict errors for biologically valid selfing
    pedigrees
...
1.  **Academic nomenclature alignment**: Updated documentation for
    ...
    and
    [`pedne()`](https://luansheng.github.io/visPedigree/reference/pedne.md)
    to distinguish additive genetic relationship ($`a_{ij}`$) from
    coancestry ($`f_{ij}`$).
    [`pedrel()`](https://luansheng.github.io/visPedigree/reference/pedrel.md)
...
1.  **Founder genome equivalents (`fg`)**: Integrated calculation of
    founder genome equivalents into
    [`pediv()`](https://luansheng.github.io/visPedigree/reference/pediv.md).
    The implementation evaluates mean coancestry with diagonal
    intra-cohort correction through adaptive scaling, keeping
...
2.  **Reproducible parameter inference**: Added a `seed` argument to
    ...
    and
    [`pediv()`](https://luansheng.github.io/visPedigree/reference/pediv.md)
    for reproducible sampling in effective population size and `fg`
    calculations that use Monte Carlo approximations.
...
1.  **Trace edge highlighting in
    [`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)**:
    Fixed incorrect edge highlighting when using `trace = "all"`. When a
    node was highlighted as both an ancestor and a parent of
    descendants, cross-path edges could be highlighted incorrectly. The
    fix separates upward and downward trace paths and uses `trace_edges`
    to control highlighted edges.
2.  **Focal-node upward edge in `trace = "down"`**: Fixed an issue where
    ...
    highlighted when tracing downward only. Individual-to-family edges
    are now highlighted only when the individual appears as a child in
    the traced path.
...
1.  **Unified diversity analysis
    ([`pediv()`](https://luansheng.github.io/visPedigree/reference/pediv.md))**:
    Added
    [`pediv()`](https://luansheng.github.io/visPedigree/reference/pediv.md)
    as a single entry point that aggregates founder contributions
    (`f_e`), ancestor contributions (`f_a`), and three `N_e` estimates
    (coancestry, inbreeding, and demographic) into a `pediv` S3 object.
    A
    [`print.pediv()`](https://luansheng.github.io/visPedigree/reference/print.pediv.md)
...
1.  **Ancestral analysis
    ([`pedcontrib()`](https://luansheng.github.io/visPedigree/reference/pedcontrib.md))**:
    Added algorithms for assessing genetic diversity through gene-origin
    probabilities.
    [`pedcontrib()`](https://luansheng.github.io/visPedigree/reference/pedcontrib.md)
    computes effective number of founders (`f_e`) through recursive gene
    derivation and effective number of ancestors (`f_a`) through
    Boichard’s iterative algorithm.
2.  **Missing-parent conservation in
    [`pedcontrib()`](https://luansheng.github.io/visPedigree/reference/pedcontrib.md)**:
    Single missing parents are augmented with temporary phantom parents
    before processing so probability mass is conserved for half-founder
    records.
3.  **Ancestry proportions
    ([`pedancestry()`](https://luansheng.github.io/visPedigree/reference/pedancestry.md))**:
    Added
    [`pedancestry()`](https://luansheng.github.io/visPedigree/reference/pedancestry.md)
    to trace line origins and monitor surviving gene proportions from
    specified historic founder lines or strains to descendants.
4.  **Partial inbreeding
    ([`pedpartial()`](https://luansheng.github.io/visPedigree/reference/pedpartial.md))**:
    Added
    [`pedpartial()`](https://luansheng.github.io/visPedigree/reference/pedpartial.md)
    to decompose the overall inbreeding coefficient into fractions
    attributed to targeted ancestors, following Meuwissen and Luo
    (1992).
...
1.  **Effective population size methods in
    [`pedne()`](https://luansheng.github.io/visPedigree/reference/pedne.md)**:
    Expanded
    [`pedne()`](https://luansheng.github.io/visPedigree/reference/pedne.md)
    to support three methods for estimating `N_e`: `"coancestry"` based
    on the rate of coancestry ($`\Delta c`$), `"inbreeding"` based on
    the individual rate of inbreeding ($`\Delta F`$), and
    `"demographic"` based on the numbers of breeding males ($`N_m`$) and
    females ($`N_f`$). The coancestry method is now the default and may
    provide earlier signals of diversity loss in selected populations
...
1.  **[`pedgenint()`](https://luansheng.github.io/visPedigree/reference/pedgenint.md)
...
6.  **[`pedpartial()`](https://luansheng.github.io/visPedigree/reference/pedpartial.md)
    ...
    incoming pedigrees, such as objects created with `addnum = FALSE`,
    no longer break
    [`pedpartial()`](https://luansheng.github.io/visPedigree/reference/pedpartial.md)
    or
    [`pedancestry()`](https://luansheng.github.io/visPedigree/reference/pedancestry.md).
...
1.  **Pedigree analysis module**: Added pedigree analysis and
    population-genetics functions:
    [`pedstats()`](https://luansheng.github.io/visPedigree/reference/pedstats.md)
    for summary and demographic statistics;
    [`pedrel()`](https://luansheng.github.io/visPedigree/reference/pedrel.md)
    for average relatedness within groups;
    [`pedgenint()`](https://luansheng.github.io/visPedigree/reference/pedgenint.md)
    for sire-sire, sire-dam, dam-sire, dam-dam, and overall generation
    intervals;
    [`pedcontrib()`](https://luansheng.github.io/visPedigree/reference/pedcontrib.md)
    for founder and ancestor genetic contributions;
    [`pedancestry()`](https://luansheng.github.io/visPedigree/reference/pedancestry.md)
    for ancestral-lineage proportions; and
    [`pedpartial()`](https://luansheng.github.io/visPedigree/reference/pedpartial.md)
    for partial inbreeding attributed to specified ancestors.
2.  **Pedigree analysis visualization**: Added
    [`vispstat()`](https://luansheng.github.io/visPedigree/reference/vispstat.md)
    for plotting generation-interval bar charts and depth-related
    distributions such as Equivalent Complete Generations.
...
1.  **Compact matrix correctness**: Fixed incorrect `A`, `D`, and `AA`
    relationship values in `compact = TRUE` mode for parent-offspring
    and avuncular pairs caused by merging parent individuals with their
    non-parent siblings.
2.  **Pedigree compression strategy**: Updated compaction logic to
    preserve the genetic identity of any individual that appears as a
    sire or dam, ensuring parents have unique entries in the
    relationship matrix.
3.  **Sibling row and column expansion**: Fixed
    [`expand_pedmat()`](https://luansheng.github.io/visPedigree/reference/expand_pedmat.md)
    so sibling off-diagonal elements are calculated from parent kinship
    rather than by duplicating representative diagonal values.
4.  **Generation alignment**: Fixed `tidyped(..., genmethod = "bottom")`
    so sibling consistency is prioritized over mate alignment, ensuring
    full siblings are aligned to the same generation.
5.  **[`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    edge highlighting**: Fixed edge highlighting so relationship edges
    are only emphasized when `trace` is used.
6.  **Shared-parent and shared-child paths**: Corrected edge
    highlighting for cases where a parent has multiple families or a
    family has multiple children.
7.  **[`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    layout with `showf = TRUE`**: Fixed a layout optimization failure by
    using immutable individual IDs.

## Changes in version 1.0.0 released on 24 Jan 2026

### Breaking changes

1.  **Matrix API names**: Renamed `pedmatrix()` to
    [`pedmat()`](https://luansheng.github.io/visPedigree/reference/pedmat.md),
    `expand_pedmatrix()` to
    [`expand_pedmat()`](https://luansheng.github.io/visPedigree/reference/expand_pedmat.md),
    and `summary_pedmatrix()` to
    [`summary_pedmat()`](https://luansheng.github.io/visPedigree/reference/summary_pedmat.md).
    Legacy function names were removed.
2.  **[`pedmat()`](https://luansheng.github.io/visPedigree/reference/pedmat.md)
    default method**: Changed the default `method` of
    [`pedmat()`](https://luansheng.github.io/visPedigree/reference/pedmat.md)
    from `"f"` to `"A"`.
...
1.  **`pedmatrix()` return value and method selection**: `pedmatrix()`
...
1.  **Flexible generation assignment**: Added the `genmethod` parameter
    to
    [`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md).
    Users can choose `"top"` (top-aligned, default) or `"bottom"`
...
1.  **User feedback**: Standardized filtering notifications. The message
    `"Note: Removed N isolated individuals..."` now appears consistently
    for all pedigree sizes when Gen 0 individuals are present.
2.  **Trace semantics**: Corrected `trace = "all"` in
    [`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md)
    and
    [`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
...
1.  **Graph-based
    [`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md)
    core**: Reimplemented the pedigree tidying engine around a directed
    acyclic graph representation, with cycle detection and generation
    inference based on topological sorting.
2.  **Modular
    [`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
...
1.  **Highlight styling**: Implemented opaque highlighting effects for
    clearer visualization.
2.  **[`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    trace direction**: Added the `trace` option to
    [`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
...
1.  **Strict `tidyped` S3 class structure**: Added the
    [`new_tidyped()`](https://luansheng.github.io/visPedigree/reference/new_tidyped.md)
    constructor and
    [`validate_tidyped()`](https://luansheng.github.io/visPedigree/reference/validate_tidyped.md)
    validator for `tidyped` objects.
...
1.  **[`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    highlighting**: Added the `highlight` parameter to
    [`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    for highlighting specific individuals with a character vector of IDs
    or a list of custom colors.
2.  **[`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    inbreeding display**: Added the `showf` parameter to
    [`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md)
    to display inbreeding coefficients on the pedigree graph.
3.  **[`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md)
    inbreeding calculation**: Added the `inbreed` parameter to
    [`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md)
    to calculate inbreeding coefficients using `nadiv`.
4.  **Standalone
    [`inbreed()`](https://luansheng.github.io/visPedigree/reference/inbreed.md)**:
    Refactored
    [`inbreed()`](https://luansheng.github.io/visPedigree/reference/inbreed.md)
...
1.  **Edge overlap detection**: Fixed overlapping edge detection for
    small pedigree graphs.
2.  **Compact-mode coloring**: Improved coloring consistency for compact
    mode.
...
1.  **Candidate generation tracing**: Fixed a bug where candidates were
    traced to generation `n + 1` when `tracegen = n`. Reported by Mianyu
    Liu.
...
1.  **`trace = "all"` in
    ...
    Fixed cases where
    [`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md)
    did not work with `trace = "all"` ([issue
    comment](https://github.com/luansheng/visPedigree/issues/2#issue-568599008)).
...
1.  **Repeated
    [`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md)
    calls**: Fixed an unexpected column named `NA` when a `tidyped`
    object was tidied again with
    [`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md).
...
1.  **Input mutation**: Fixed a bug where a `data.table` supplied as
    `ped` could be changed by
    [`tidyped()`](https://luansheng.github.io/visPedigree/reference/tidyped.md)
    or
    [`visped()`](https://luansheng.github.io/visPedigree/reference/visped.md).
...
1.  **Generation inference**: Fixed incorrect generation-number
    inference for individuals.

## Changes in version 0.2.2 released on 28 Jan 2019

### Bug fixes

1.  **Candidate retention**: Fixed a bug where a tidied pedigree did not
    include candidates absent from the original `Ind` column when `cand`
    was not `NULL`.
...
1.  **Node overlap**: Repelled overlapping nodes caused by very small
    differences (digits \> 7) among node x positions.
